...
function isMissingFitnessSetsTable(error){
  ...
  return /42P01|PGRST205/i.test(message)||(/fitness_set_logs/i.test(message)&&/does not exist|schema cache|could not find/i.test(message));
}

function latestCompletedFitnessSession(planId,excludeId=null){
  return fitnessSessions.find(session=>session.status==='completed'&&session.plan_id===planId&&session.id!==excludeId)||null;
}

function setLogsForSession(sessionId){
  return fitnessSetLogs.filter(set=>set.session_id===sessionId).sort((a,b)=>a.set_number-b.set_number);
}

function setLogsForExercise(sessionId,planExerciseId){
  return fitnessSetLogs.filter(set=>set.session_id===sessionId&&set.plan_exercise_id===planExerciseId).sort((a,b)=>a.set_number-b.set_number);
}
...
async function loadFitness(){
  ...
  if(sessionIds.length){
    const {data,error}=await sb.from('fitness_set_logs').select('*').in('session_id',sessionIds).order('set_number',{ascending:true});
    if(error){
...
async function loadPreviousFitnessWorkout(planId){
  ...
  const [exerciseResult,setResult]=await Promise.all([
    sb.from('fitness_session_exercises').select('*').eq('session_id',session.id).order('position',{ascending:true}),
    sb.from('fitness_set_logs').select('*').eq('session_id',session.id).order('set_number',{ascending:true})
  ]);
...
async function backfillActiveFitnessSets(){
  if(!activeFitnessSession||!fitnessSetsReady||!fitnessSessionExercises.length||setLogsForSession(activeFitnessSession.id).length)return;
  ...
  for(const sessionExercise of fitnessSessionExercises){
    const previousExercise=previous.exercises.find(item=>item.plan_exercise_id===sessionExercise.plan_exercise_id);
    for(let setNumber=1;setNumber<=Number(sessionExercise.target_sets);setNumber+=1){
      const previousSet=previous.sets.find(item=>item.plan_exercise_id===sessionExercise.plan_exercise_id&&Number(item.set_number)===setNumber);
      const previousWeight=previousSet?Number(previousSet.weight_kg):(previousExercise?Number(previousExercise.actual_weight):Number(sessionExercise.actual_weight)||0);
      const previousReps=previousSet&&Number(previousSet.actual_reps)>0?Number(previousSet.actual_reps):Number(sessionExercise.target_reps);
      rows.push({
        user_id:currentUser.id,
...
async function startFitnessWorkout(planId=selectedFitnessPlanId){
  if(!fitnessReady)return alert('Bitte zuerst die Fitness-Migration in Supabase ausführen.');
  if(!fitnessSetsReady)return alert('Für die Satz-Aufzeichnung fehlt noch die Fitness-v2-Migration in Supabase.');
  ...
  let previous;
  try{previous=await loadPreviousFitnessWorkout(plan.id)}
  ...
  const {data:createdExercises,error:exerciseError}=await sb.from('fitness_session_exercises').insert(exerciseRows).select();
  if(exerciseError){
  ...
  for(const sessionExercise of createdExercises||[]){
    ...
    for(let setNumber=1;setNumber<=Number(sessionExercise.target_sets);setNumber+=1){
      const previousSet=previous.sets.find(item=>item.plan_exercise_id===sessionExercise.plan_exercise_id&&Number(item.set_number)===setNumber);
      const previousWeight=previousSet?Number(previousSet.weight_kg):(previousExercise?Number(previousExercise.actual_weight):null);
      const previousReps=previousSet&&Number(previousSet.actual_reps)>0?Number(previousSet.actual_reps):Number(sessionExercise.target_reps);
      setRows.push({
        user_id:currentUser.id,
        session_id:session.id,
...
async function updateFitnessSet(id,field,value){
  ...
  const numeric=field==='actual_reps'?Math.max(0,Math.min(200,Math.round(Number(value)||0))):Math.max(0,Number(value)||0);
  const {error}=await sb.from('fitness_set_logs').update({[field]:numeric,updated_at:new Date().toISOString()}).eq('id',id);
  if(error)return alert(error.message);
...
async function toggleFitnessSet(id,isCompleted){
  ...
  const {error}=await sb.from('fitness_set_logs').update({is_completed:isCompleted,completed_at:completedAt,updated_at:new Date().toISOString()}).eq('id',id);
  if(error)return alert(error.message);
  set.is_completed=isCompleted;
  set.completed_at=completedAt;

  const exerciseSets=fitnessSetLogs.filter(item=>item.session_exercise_id===set.session_exercise_id);
  const exerciseComplete=exerciseSets.length>0&&exerciseSets.every(item=>item.is_completed);
  const completedWeights=exerciseSets.filter(item=>item.is_completed).map(item=>Number(item.weight_kg)||0);
  const actualWeight=completedWeights.length?Math.max(...completedWeights):0;
  const {error:exerciseError}=await sb.from('fitness_session_exercises').update({
...
async function finishFitnessWorkout(){
  ...
  if(completedSets.length<activeSets.length&&!confirm('Training mit offenen Sätzen abschließen?'))return;

  const weightUpdates=fitnessSessionExercises.filter(exercise=>exercise.plan_exercise_id).map(exercise=>{
    const weights=activeSets.filter(set=>set.session_exercise_id===exercise.id&&set.is_completed).map(set=>Number(set.weight_kg)||0);
    if(!weights.length)return Promise.resolve({error:null});
    return sb.from('fitness_plan_exercises').update({target_weight:Math.max(...weights),updated_at:new Date().toISOString()}).eq('id',exercise.plan_exercise_id);
  });
...
function formatFitnessWeight(value){
  return Number(value||0).toLocaleString('de-AT',{minimumFractionDigits:0,maximumFractionDigits:2});
}
...
function renderSetSummary(sets){
  ...
  const labels=completed.map(set=>`${formatFitnessWeight(set.weight_kg)} kg × ${Number(set.actual_reps)||0}`);
      ...
      ${fitnessSessionExercises.map(exercise=>{
        ...
        return `<article class="workout-exercise ${done?'done':''}">
          ...
          <div class="workout-set-list">
            ${sets.map(set=>`<div class="workout-set-row ${set.is_completed?'done':''}">
...
function openFitnessProgress(exerciseId){
  ...
  if(!exercise)return;
  const sessions=fitnessSessions.filter(session=>session.status==='completed'&&session.plan_id===exercise.plan_id);
  const entries=sessions.map(session=>({session,sets:setLogsForExercise(session.id,exercise.id).filter(set=>set.is_completed)})).filter(entry=>entry.sets.length);
  $('#fitnessProgressTitle').textContent=exercise.name;
  $('#fitnessProgressContent').innerHTML=entries.length?entries.map((entry,index)=>{
    const currentMax=Math.max(...entry.sets.map(set=>Number(set.weight_kg)||0));
    const older=entries[index+1];
    const olderMax=older?.sets.length?Math.max(...older.sets.map(set=>Number(set.weight_kg)||0)):null;
    const change=olderMax!==null&&currentMax>olderMax?`<span class="progress-gain">+${formatFitnessWeight(currentMax-olderMax)} kg</span>`:'';
  ...
  }).join(''):'<div class="empty">Nach dem ersten abgeschlossenen Training erscheint hier dein satzgenauer Verlauf.</div>';
...
function renderFitness(){
  ...
  $('#fitnessStartBtn').disabled=!fitnessReady||!fitnessSetsReady||!fitnessPlans.length||Boolean(activeFitnessSession);
  ...
  $('#fitnessLastWorkout').textContent=lastPlanSession?`${formatFitnessDate(lastPlanSession.completed_at||lastPlanSession.session_date)} · ${selectedPlan.name}`:'–';
  ...
  if(fitnessReady&&fitnessPlans.length){
    ...
    $('#fitnessPlanList').innerHTML=rows.length?rows.map((exercise,index)=>{
      const lastSets=lastPlanSession?setLogsForExercise(lastPlanSession.id,exercise.id):[];
