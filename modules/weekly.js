...
function isMissingWeeklyReviewTable(error){
  ...
  return text.includes('42p01')||text.includes('pgrst205')||text.includes('weekly_reviews')&&text.includes('not find');
}
...
async function loadWeeklyReview(){
  ...
  if(taskResult.error){
    if(typeof isMissingDailyTasksTable!=='function'||!isMissingDailyTasksTable(taskResult.error))throw taskResult.error;
    weeklyTasks=[];
  }else{
...
function weeklyMetrics(){
  ...
  const weekSessions=fitnessSessions.filter(session=>session.status==='completed'&&inRange(session.session_date||session.completed_at));
...
function weeklyRangeLabel(bounds){
  const start=new Intl.DateTimeFormat('de-DE',{day:'2-digit',month:'short'}).format(bounds.startDate);
  const end=new Intl.DateTimeFormat('de-DE',{day:'2-digit',month:'short',year:'numeric'}).format(bounds.endDate);
  return `${start} – ${end}`;
}
...
function weeklyInsightMarkup(metrics){
  ...
  if(!metrics.weekTrades.length){
    insights.push({icon:'📈',tone:'neutral',title:'Trading bewusst halten',copy:'Noch keine Trades in dieser Woche – Qualität bleibt wichtiger als Aktivität.'});
  }else if(metrics.planRate>=80){
    insights.push({icon:'✓',tone:'positive',title:'Starke Trading-Disziplin',copy:`${metrics.planRate.toFixed(0)}% deiner Trades folgten dem Plan.`});
  }else{
    insights.push({icon:'◎',tone:'warning',title:'Prozess vor Ergebnis',copy:`${metrics.planRate.toFixed(0)}% Plan-Treue – hier liegt dein größter Trading-Hebel.`});
  }

  if(metrics.weekSessions.length>=2){
    insights.push({icon:'💪',tone:'positive',title:'Trainingsziel erreicht',copy:`${metrics.weekSessions.length} Einheiten bringen Konstanz in deine Woche.`});
  }else{
    insights.push({icon:'💪',tone:'warning',title:'Training offen',copy:`Noch ${Math.max(0,2-metrics.weekSessions.length)} Einheit${metrics.weekSessions.length===1?'':'en'} bis zum Wochenziel.`});
  }
  ...
  if(!weeklyTasks.length){
    ...
    insights.push({icon:'◎',tone:'warning',title:'Prioritäten schärfen',copy:`${metrics.completedTasks} von ${weeklyTasks.length} Aufgaben erledigt – weniger kann mehr sein.`});
  }

  const leadGoal=goals.find(goal=>goal.next_action)||goals[0];
  if(leadGoal){
    insights.push({icon:'🎯',tone:'neutral',title:'Nächster Ziel-Schritt',copy:leadGoal.next_action||leadGoal.title});
  }

  return insights.map(item=>`<div class="weekly-insight insight-${item.tone}"><span>${item.icon}</span><div><b>${escapeHtml(item.title)}</b><small>${escapeHtml(item.copy)}</small></div></div>`).join('');
}

function renderWeeklyReview(){
  const metrics=weeklyMetrics();
  const scoreTitle=metrics.score>=85?'Starke Woche':metrics.score>=65?'Stabil unterwegs':metrics.score>=40?'Im Aufbau':'Jetzt Kurs setzen';
  const scoreCopy=metrics.score>=65?'Deine Systeme greifen ineinander. Halte den Prozess einfach und wiederholbar.':'Der Review zeigt dir klar, wo der nächste kleine Hebel liegt.';

  $('#weeklyRange').textContent=weeklyRangeLabel(metrics.bounds);
  $('#weeklyRangeStatus').textContent=weeklyOffset===0?'Diese Woche':weeklyOffset===-1?'Letzte Woche':`${Math.abs(weeklyOffset)} Wochen zurück`;
  ...
  $('#weeklyTradeMeta').textContent=metrics.weekTrades.length?`${metrics.weekTrades.length} Trades · ${metrics.winrate.toFixed(0)}% Winrate · ${metrics.planRate.toFixed(0)}% Plan`:'Noch keine Trades';
  ...
  $$('#weeklyForm input,#weeklyForm textarea,#weeklyForm button').forEach(field=>field.disabled=!weeklyReady);
}
...
$('#weeklyForm')?.addEventListener('submit',async event=>{
  ...
  const {error}=await sb.from('weekly_reviews').upsert(payload,{onConflict:'user_id,week_start'});
  if(error){
