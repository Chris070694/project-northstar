...
function isMissingGoalImageSchema(error) {
  ...
  return (
    message.includes('image_path') && /column|schema cache|does not exist|not find/.test(message)
  );
}
...
$('#goalForm').onsubmit = async event => {
  ...
  try {
    ...
    alert(
      isMissingGoalImageSchema(error)
...
async function renderGoals() {
  ...
  $('#longGoals').innerHTML =
    longCards.join('') || '<div class="empty">Noch keine Long-Term-Ziele.</div>';
  $('#shortGoals').innerHTML =
    shortCards.join('') || '<div class="empty">Noch keine Short-Term-Ziele.</div>';
  $('#homeGoals').innerHTML =
    homeCards.join('') || '<div class="empty">Lege dein erstes Ziel an.</div>';
  if (longGoals[0]) {
    const imageUrl = await signedUrl(longGoals[0].image_path);
    $('#heroGoal').style.backgroundImage = imageUrl ? `url("${imageUrl}")` : 'none';
    $('#heroGoal').innerHTML =
      `<div class="vision-content"><span class="pill">DEIN NORDSTERN</span><h2>${longGoals[0].title}</h2><p class="sub">${longGoals[0].next_action || longGoals[0].why_text || ''}</p></div>`;
  }
}
