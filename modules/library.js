...
async function loadLibrary(){
  const result=await sb.from('library_books').select('*')
...
$('#bookPdf')?.addEventListener('change',event=>{
  ...
  if(!$('#bookTitle').value.trim())$('#bookTitle').value=file.name.replace(/\.pdf$/i,'').replace(/[_-]+/g,' ');
});
...
async function inspectLibraryPdf(file){
  ...
  try{
    ...
    coverBlob=await new Promise(resolve=>canvas.toBlob(resolve,'image/webp',.82));
  }catch(error){
...
$('#libraryBookForm')?.addEventListener('submit',async event=>{
  ...
  if(file.type!=='application/pdf'&&!file.name.toLowerCase().endsWith('.pdf'))return alert('Bitte nur PDF-Dateien hochladen.');
  ...
  try{
    ...
    await uploadLibraryPdf(file,pdfPath,percent=>setLibraryUploadProgress(Math.max(5,Math.round(percent*.9)),`Upload ${percent}%`));
    pdfUploaded=true;
...
async function renderLibrary(){
  ...
  $('#libraryReadingCount').textContent=String(libraryBooks.filter(book=>Number(book.current_page)>1&&libraryProgress(book)<100).length);
  $('#libraryFinishedCount').textContent=String(libraryBooks.filter(book=>libraryProgress(book)>=100).length);

  if(!libraryReady){
    $('#libraryContinue').classList.add('hide');
    grid.innerHTML='<div class="empty">Nach der Supabase-Einrichtung kannst du hier deine PDFs hochladen.</div>';
    return;
  }

  const query=($('#librarySearch')?.value||'').trim().toLowerCase();
  const category=$('#libraryCategoryFilter')?.value||'';
  const filtered=libraryBooks.filter(book=>(!category||book.category===category)&&
  ...
  if(latest){
    const latestCover=latest.cover_path?(covers.get(latest.cover_path)||await librarySignedUrl(latest.cover_path)):'';
    continueCard.classList.remove('hide');
    $('#libraryContinueCover').innerHTML=latestCover?`<img src="${escapeHtml(latestCover)}" alt="">`:'<span>📖</span>';
    $('#libraryContinueTitle').textContent=latest.title;
    $('#libraryContinueMeta').textContent=`Seite ${latest.current_page||1} von ${latest.total_pages||1} · ${libraryProgress(latest)}%`;
  ...
  }).join(''):'<div class="empty">Keine Bücher gefunden. Lade deine erste PDF hoch.</div>';
}
...
async function openLibraryBook(id){
  ...
  libraryReaderPage=Math.min(Math.max(1,Number(book.current_page)||1),Math.max(1,Number(book.total_pages)||1));
  libraryReaderZoom=1;
  ...
  try{
    ...
    $('#libraryOpenOriginalBtn').onclick=()=>window.open(activeLibraryPdfUrl,'_blank','noopener');
...
async function renderLibraryReaderPage(){
  ...
  try{activeLibraryRenderTask?.cancel()}catch(_error){}
  $('#libraryReaderLoading').classList.remove('hide');
  ...
  activeLibraryRenderTask=page.render({canvasContext:canvas.getContext('2d'),viewport:renderViewport});
  try{await activeLibraryRenderTask.promise}catch(error){if(error?.name!=='RenderingCancelledException')throw error}
  if(renderId!==libraryRenderVersion)return;
...
async function saveLibraryProgress(){
  ...
  const {error}=await sb.from('library_books').update({
...
async function goToLibraryPage(){
  if(!activeLibraryPdf)return;
  const next=Math.min(activeLibraryPdf.numPages,Math.max(1,Number($('#libraryPageInput').value)||1));
  libraryReaderPage=next;
  await renderLibraryReaderPage();
}
...
async function changeLibraryZoom(delta){
  libraryReaderZoom=Math.min(2.5,Math.max(.65,libraryReaderZoom+delta));
  updateLibraryZoomLabel();
  await renderLibraryReaderPage();
}
...
async function closeLibraryReader(){
  ...
  try{activeLibraryRenderTask?.cancel()}catch(_error){}
  await saveLibraryProgress();
  try{await activeLibraryPdf?.destroy()}catch(_error){}
