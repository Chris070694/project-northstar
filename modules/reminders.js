...
async function loadReminderSettings() {
  ...
  pushSubscription =
    (subscriptionResult.data || []).find(item => item.endpoint === localPushEndpoint) || null;
}
...
function renderReminderSettings() {
  ...
  $$('.fitness-reminder-day').forEach(input => {
    input.checked = fitnessDays.has(Number(input.value));
  });
  ...
  status.className =
    'notification-state ' +
    (active ? 'active' : capability.permission === 'denied' ? 'blocked' : '');
  ...
  if (!capability.supported) {
    ...
    copy.textContent =
      'Auf dem iPhone muss CPRB zuerst zum Home-Bildschirm hinzugefügt und von dort geöffnet werden.';
  } else if (capability.permission === 'denied') {
    copy.textContent = 'Benachrichtigungen sind in den Geräte-Einstellungen blockiert.';
  } else if (active) {
    copy.textContent =
      'Dieses Gerät empfängt deine CPRB-Erinnerungen – auch wenn die App geschlossen ist.';
  } else {
    copy.textContent =
      'Aktiviere Push einmal auf diesem Gerät. CPRB fragt erst nach deinem Klick um Erlaubnis.';
  }
  ...
  $$('#reminderForm input,#reminderForm select,#reminderForm button').forEach(field => {
    if (
      field.id !== 'enableNotificationsBtn' &&
      field.id !== 'disableNotificationsBtn' &&
      field.id !== 'testNotificationBtn'
    )
      field.disabled = !reminderSettingsReady;
  });
}
...
async function getPushPublicKey() {
  const { data, error } = await sb.functions.invoke('smart-reminder', {
    body: { action: 'public-key' },
  });
  if (error) throw new Error('Der Push-Server ist noch nicht aktiviert.');
  if (!data?.publicKey)
    throw new Error('Der Push-Server hat keinen öffentlichen Schlüssel geliefert.');
...
async function enableSmartNotifications() {
  ...
  const capability = notificationCapability();
  if (!capability.supported)
    return alert('Web Push wird auf diesem Gerät oder Browser nicht unterstützt.');
  if (!capability.standalone && /iphone|ipad|ipod/i.test(navigator.userAgent)) {
    return alert(
      'Bitte CPRB zuerst zum Home-Bildschirm hinzufügen, von dort öffnen und dann Push aktivieren.',
    );
  }
  if (Notification.permission === 'denied')
    return alert('Bitte Benachrichtigungen in den Geräte-Einstellungen für CPRB erlauben.');
