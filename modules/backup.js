...
async function collectBackupData(
  tableNames = CPRB_BACKUP_TABLES.map(item => item.name),
  onProgress = () => {},
) {
  const tables = {};
  const unavailable = [];
  ...
  for (let index = 0; index < fileRefs.length; index += 1) {
    ...
    try {
      ...
      included.push({
        ...
        zip_path: zipPath,
        size: blob.size,
        type: backupFileMimeType(file.path, blob.type),
      });
...
async function deriveBackupKey(password, salt, usage) {
  ...
  return crypto.subtle.deriveKey(
    { name: 'PBKDF2', salt, iterations: CPRB_BACKUP_ITERATIONS, hash: 'SHA-256' },
    material,
...
async function encryptBackupBytes(bytes, password) {
  ...
  const encrypted = new Uint8Array(
    await crypto.subtle.encrypt({ name: 'AES-GCM', iv, additionalData: magic }, key, bytes),
  );
...
async function decryptBackupBytes(bytes, password) {
  ...
  if (bytes.length < magic.length + 16 + 12 + 16)
    throw new Error('Die Datei ist kein gültiges CPRB-Backup.');
  ...
  try {
    ...
    return new Uint8Array(
      await crypto.subtle.decrypt({ name: 'AES-GCM', iv, additionalData: magic }, key, encrypted),
    );
  } catch (error) {
...
async function createCprbBackup(includeFiles = true) {
  ...
  if (password.length < 6)
    return alert('Bitte ein Backup-Passwort mit mindestens 6 Zeichen eingeben.');
  if (!window.JSZip)
    return alert('Das Backup-Modul konnte nicht geladen werden. Bitte die App neu öffnen.');
  if (
    includeFiles &&
    !confirm(
      'Das komplette Backup kann wegen deiner PDFs größer werden. Am Windows-Laptop funktioniert der Download am zuverlässigsten. Jetzt starten?',
    )
  )
    return;
  ...
  try {
    const collected = await collectBackupData(undefined, (part, name) =>
      setBackupProgress('backup', 3 + part * 26, `${name} wird gesichert…`),
    );
    const zip = new JSZip();
    ...
    if (includeFiles) {
      ...
      files = await addFilesToBackup(zip, refs, (index, total, path) => {
        ...
        setBackupProgress(
          'backup',
          30 + part * 45,
          `Dateien ${index + 1}/${total}: ${path.split('/').pop()}`,
        );
      });
    }
    const manifest = {
      ...
      local: {
        active_fitness_plan:
          localStorage.getItem(`northstar-fitness-plan-${currentUser.id}`) || null,
      },
    ...
    const archive = await zip.generateAsync(
      ...
      metadata => {
        setBackupProgress(
          'backup',
          78 + metadata.percent * 0.1,
          `Backup wird gepackt… ${Math.round(metadata.percent)}%`,
        );
      },
    );
    ...
    setBackupProgress(
      'backup',
      100,
      `${collected.rowCount} Einträge${includeFiles ? ` und ${files.included.length} Dateien` : ''} gesichert · ${formatBackupBytes(encrypted.length)}`,
    );
    if (files.failed.length)
      alert(
        `Backup erstellt. ${files.failed.length} Datei(en) konnten nicht geladen werden und fehlen in dieser Sicherung.`,
      );
  } catch (error) {
    console.error(error);
...
async function restoreCprbBackup() {
  ...
  if (
    !confirm(
      'Backup jetzt sicher mit deinen vorhandenen Daten zusammenführen? Es werden keine Einträge gelöscht.',
    )
  )
    return;
  ...
  try {
    ...
    if (manifest.format !== 'cprb-backup' || manifest.version !== CPRB_BACKUP_VERSION)
      throw new Error('Diese Backup-Version wird noch nicht unterstützt.');
    if (manifest.owner_id !== currentUser.id)
      throw new Error('Dieses Backup gehört zu einem anderen CPRB-Konto.');
    ...
    if (manifest.local?.active_fitness_plan)
      localStorage.setItem(
        `northstar-fitness-plan-${currentUser.id}`,
        manifest.local.active_fitness_plan,
      );
    ...
    setBackupProgress(
      'restore',
      100,
      `${restored} Einträge und ${files.length} Dateien wiederhergestellt ✓`,
    );
...
async function exportCprbCsv() {
  ...
  try {
    ...
    const zip = new JSZip();
    Object.entries(collected.tables).forEach(([name, rows]) =>
...
function renderBackupCenter() {
  ...
  title.textContent =
    days === 0
      ...
      : days === 1
        ? 'Gestern gesichert'
        : `Vor ${days} Tagen gesichert`;
  copy.textContent = due
    ? 'Eine neue Sicherung ist empfohlen.'
...
$('#backupFile')?.addEventListener('change', event => {
  ...
  $('#backupFileName').textContent = file
    ? `${file.name} · ${formatBackupBytes(file.size)}`
    : 'CPRB-Backup auswählen';
});
