...
function isMissingPencilNotesSchema(error){
  ...
  return /note_type|drawing_path|paper_style|updated_at/.test(message)&&/does not exist|schema cache|not find|pgrst204|42703/.test(message);
}
...
async function loadNotes(){
  const columns='id,user_id,created_at,updated_at,title,category,content,image_path,note_type,drawing_path,paper_style';
  ...
  notes=(data||[]).map(note=>({note_type:'text',drawing_path:null,paper_style:'lined',...note}));
  await Promise.all(notes.filter(note=>note.note_type==='handwriting'&&note.image_path).map(async note=>{
...
function pencilCanvas(){return $('#pencilCanvas')}
...
async function openPencilNote(noteId=null){
  if(!pencilNotesReady)return alert('Bitte zuerst die Pencil-Notizen-Migration in Supabase ausführen.');
  resetPencilEditor();
...
async function loadPencilDrawing(note){
  ...
  try{
    ...
    if(Number(drawing.version)!==PENCIL_DRAWING_VERSION||!Array.isArray(drawing.strokes))throw new Error('Unbekanntes Pencil-Format');
...
function updatePencilToolbar(){
  $$('.pencil-tool').forEach(button=>button.classList.toggle('active',button.dataset.tool===pencilTool));
  $$('.pencil-color').forEach(button=>button.classList.toggle('active',button.dataset.color===pencilColor));
...
function pencilStrokeWidth(size,pressure,tool='pen'){
  ...
  const force=Number(pressure)>0?Number(pressure):.45;
  return base*(.38+Math.min(1,force)*1.05);
}
...
function pencilPoint(event){
  ...
  return {
    ...
    pressure:event.pointerType==='mouse'?.5:(Number(event.pressure)>0?Number(event.pressure):.45)
  };
}
...
function drawPaper(context,style){
  ...
  if(style==='lined'||style==='grid'){
    ...
    for(let y=82;y<PENCIL_CANVAS_HEIGHT;y+=54){
      context.beginPath();context.moveTo(0,y);context.lineTo(PENCIL_CANVAS_WIDTH,y);context.stroke();
    }
  }
  if(style==='grid'){
    for(let x=54;x<PENCIL_CANVAS_WIDTH;x+=54){
      context.beginPath();context.moveTo(x,0);context.lineTo(x,PENCIL_CANVAS_HEIGHT);context.stroke();
    }
  }
  if(style==='dotted'){
  ...
  if(style==='lined'){
    ...
    context.beginPath();context.moveTo(104,0);context.lineTo(104,PENCIL_CANVAS_HEIGHT);context.stroke();
  }
  context.restore();
}
...
function drawPencilStroke(context,stroke){
  ...
  if(points.length===1){
    context.beginPath();
    context.arc(points[0].x,points[0].y,pencilStrokeWidth(stroke.size,points[0].pressure,stroke.tool)/2,0,Math.PI*2);
    context.fill();
  }else{
    for(let index=1;index<points.length;index+=1){
      const previous=points[index-1],current=points[index];
      context.lineWidth=pencilStrokeWidth(stroke.size,(previous.pressure+current.pressure)/2,stroke.tool);
...
function canvasBlob(canvas,type='image/png',quality=.92){
  return new Promise((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('Vorschau konnte nicht erstellt werden.')),type,quality));
}

async function uploadPencilAsset(path,blob,contentType){
  const {error}=await sb.storage.from('northstar-media').upload(path,blob,{upsert:true,contentType,cacheControl:'3600'});
  if(error)throw error;
}
...
async function savePencilNote(){
  ...
  try{
    renderPencilCanvas();
    const drawingBlob=new Blob([JSON.stringify({
    ...
    console.error(error);
    if(!pencilEditorNoteId)await sb.storage.from('northstar-media').remove([drawingPath,imagePath]);
...
async function deleteNote(id){
  ...
  if(paths.length){
    ...
    if(cleanup.error)console.warn('Notiz-Dateien konnten nicht vollständig gelöscht werden.',cleanup.error);
  }
  await loadAll();
}
...
function renderNotes(){
  ...
  $('#notesList').innerHTML=notes.length?notes.map(note=>note.note_type==='handwriting'?`
    ...
    </article>`).join(''):'<div class="empty">Noch keine Notizen.</div>';
}
...
$('#pencilPaper').addEventListener('change',()=>{pencilDirty=true;renderPencilCanvas()});
['#pencilTitle','#pencilCategory'].forEach(selector=>$(selector).addEventListener('input',()=>{pencilDirty=true}));
