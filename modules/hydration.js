...
async function loadHydration(){
  ...
  const [settingsResult,dayResult]=await Promise.all([
    sb.from('hydration_settings').select('daily_goal_ml').eq('user_id',currentUser.id).maybeSingle(),
    sb.from('hydration_days').select('amount_ml').eq('user_id',currentUser.id).eq('day',day).maybeSingle()
  ]);
...
function renderHydration(){
  ...
  document.getElementById('hydrationGoal').textContent=`${(hydrationGoalMl/1000).toFixed(2).replace('.',',')} L`;
  ...
  document.querySelectorAll('#hydrationCard button').forEach(button=>button.disabled=!hydrationReady);
}
...
async function addHydration(delta){
  ...
  const {data,error}=await sb.from('hydration_days').upsert({
    user_id:currentUser.id,
    day:hydrationDateKey(),
    amount_ml:next,
    updated_at:new Date().toISOString()
  },{onConflict:'user_id,day'}).select('amount_ml').single();
...
async function setHydrationGoal(){
  ...
  if(!Number.isFinite(liters)||liters<0.5||liters>10)return alert('Bitte einen Wert zwischen 0,5 und 10 Litern eingeben.');
  const goal=Math.round(liters*1000);
  const {data,error}=await sb.from('hydration_settings').upsert({
    user_id:currentUser.id,
    daily_goal_ml:goal,
    updated_at:new Date().toISOString()
  },{onConflict:'user_id'}).select('daily_goal_ml').single();
  if(error)return alert(error.message);
