...
async function loadAcademy(){
  const {data,error}=await sb.from('academy_notes').select('*').order('updated_at',{ascending:false});
  if(error)throw error;
  academyNotes=data||[];
}

function openAcademyNote(){ $('#academyModal').classList.add('open'); }
function closeAcademyNote(){ $('#academyModal').classList.remove('open'); }
function closeAcademyDetail(){ $('#academyDetailModal').classList.remove('open'); }

$('#academyForm').addEventListener('submit',async e=>{
  e.preventDefault();
  try{
    const image_path=await uploadMediaToFolder($('#aImage').files[0],'academy');
    const tags=$('#aTags').value.split(',').map(x=>x.trim()).filter(Boolean);
    const {error}=await sb.from('academy_notes').insert({
      user_id:currentUser.id,title:$('#aTitle').value,category:$('#aCategory').value,
      definition:$('#aDefinition').value,rules:$('#aRules').value,
      example_text:$('#aExample').value,mistakes:$('#aMistakes').value,
      checklist:$('#aChecklist').value,tags,image_path,updated_at:new Date().toISOString()
    });
    if(error)throw error;
    e.target.reset();closeAcademyNote();await loadAll();showPage('academy');
...
async function renderAcademy(){
  ...
  const list=academyNotes.filter(n=>(!cat||n.category===cat)&&(`${n.title} ${n.definition||''} ${(n.tags||[]).join(' ')}`.toLowerCase().includes(q)));
  const cards=[];
  for(const n of list){
  ...
  $('#academyGrid').innerHTML=cards.join('')||'<div class="empty">Noch keine Lernnotizen. Starte beispielsweise mit MSS.</div>';
}
