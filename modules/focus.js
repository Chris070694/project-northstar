...
function isMissingDailyTasksTable(error) {
  ...
  return (
    error?.code === '42P01' ||
    (message.includes('daily_tasks') && /does not exist|schema cache/i.test(message))
  );
}
...
function isMissingRecurringTasksSchema(error) {
  ...
  return (
    ...
    (message.includes('source_recurring_task_id') &&
...
async function loadFocus() {
  ...
  const [taskResult, priorityResult, persistentResult, recurringResult, skipResult] =
    await Promise.all([
      sb
        .from('daily_tasks')
        .select('*')
  ...
  if (taskResult.error) {
    if (
      ...
      !isMissingRecurringTasksSchema(taskResult.error)
    )
  ...
  if (priorityResult.error) {
    if (
      !isMissingTaskPrioritySchema(priorityResult.error) &&
      !isMissingDailyTasksTable(priorityResult.error)
    )
  ...
  if (persistentResult.error) {
    if (
      !isMissingPersistentTasksSchema(persistentResult.error) &&
      !isMissingDailyTasksTable(persistentResult.error)
    )
      throw persistentResult.error;
    persistentTasksReady = false;
  } else {
  ...
  recurringTasks = (recurringResult.data || []).filter(
    task => task.starts_on <= d && (!task.ends_on || task.ends_on >= d),
  );
  if (skipResult.error) {
  ...
  const existingSources = new Set(
    dailyTasks.map(task => task.source_recurring_task_id).filter(Boolean),
  );
  const missing = recurringTasks.filter(
    task => !existingSources.has(task.id) && !skippedSources.has(task.id),
  );
  if (missing.length) {
    const basePosition = dailyTasks.length
      ? Math.max(...dailyTasks.map(task => Number(task.position) || 0)) + 1
      : 0;
...
$('#taskForm').onsubmit = async e => {
  e.preventDefault();
  if (!dailyTasksReady)
    return alert('Bitte zuerst die daily_tasks-Migration in Supabase ausführen.');
  ...
  if (keepUntilDone && !persistentTasksReady)
    return alert('Bitte zuerst die Migration für offene Aufgaben in Supabase ausführen.');
  if (repeats) {
    if (!recurringTasksReady)
      return alert('Bitte zuerst die Habits-&-Reminders-Migration in Supabase ausführen.');
    const recurringPosition = recurringTasks.length
      ? Math.max(...recurringTasks.map(task => Number(task.position) || 0)) + 1
      : 0;
    const recurringResult = await sb
      .from('recurring_tasks')
      .insert({
        user_id: currentUser.id,
        title,
...
async function toggleTaskPriority(id) {
  if (!taskPriorityReady)
    return alert('Bitte zuerst die To-do-Prioritäts-Migration in Supabase ausführen.');
  const task = dailyTasks.find(item => item.id === id);
  if (!task) return;
  ...
  if (error) {
    ...
    dailyTasks.forEach(item => {
      item.is_priority = item.id === id ? nextValue : false;
    });
  }
  renderFocus();
}
...
function renderRecurringTasks() {
  ...
  if (!recurringTasksReady) {
    container.innerHTML =
      '<div class="recurring-empty">Nach der Habits-Migration kannst du tägliche Aufgaben anlegen.</div>';
    return;
  }
  container.innerHTML = recurringTasks.length
    ? recurringTasks
        .map(
          task => `
  ...
  `,
        ...
        .join('')
    : '<div class="recurring-empty">Noch keine täglichen Aufgaben. Aktiviere beim Hinzufügen „Jeden Tag wiederholen“.</div>';
}
...
function renderDailyTasks() {
  ...
  $('#persistentTaskSetupNotice')?.classList.toggle(
    'hide',
    persistentTasksReady || !dailyTasksReady,
  );
  $('#taskKeepUntilDone').disabled = !persistentTasksReady;

  if (!dailyTasksReady) {
    list.innerHTML =
      '<div class="empty">Nach der Supabase-Einrichtung erscheinen hier deine Aufgaben.</div>';
  ...
  if (!total) {
    ...
    homeList.innerHTML =
      '<div class="home-task-empty">Noch keine Aufgaben – plane deinen Tag.</div>';
    renderRecurringTasks();
    return;
  }
  ...
  const openMarkup = openTasks.length
    ? openTasks.map(dailyTaskMarkup).join('')
    : '<div class="empty task-all-done">Alles erledigt ✓</div>';
  const completedMarkup = completedTasks.length
    ? `<details class="task-completed-group"><summary>Erledigt (${completedTasks.length})</summary><div>${completedTasks.map(dailyTaskMarkup).join('')}</div></details>`
    : '';
  list.innerHTML = openMarkup + completedMarkup;

  const homeTasks = openTasks.slice(0, 5);
  homeList.innerHTML = homeTasks.length
    ? homeTasks
        .map(
          task => `
  ...
  `,
        ...
        .join('')
    : '<div class="home-task-empty">Alles erledigt für heute ✓</div>';
  renderRecurringTasks();
}
